...
def analyze_chat():
    try:
        ...
        try:
            ...
            return jsonify({"chat_result": chat_html})
...
def analyze_vtt():
    try:
        ...
        curriculum_filepath = os.path.join(
            app.config["UPLOAD_FOLDER"], curriculum_filename
        )
            ...
            return jsonify(
                {
                    ...
                    "non_topic_blocks_count": len(
                        preprocess_result["non_topic_blocks"]
                    ),
...
def format_vtt_analysis(content):
    ...
    # 각 섹션의 내용을 저장할 딕셔너리
    vtt_sections = {"주요 내용": [], "키워드": [], "분석": [], "위험 발언": []}
    ...
    if vtt_sections["키워드"]:
        ...
        html_content.extend(["        </ul>", "    </div>", "</div>"])
    ...
    for risk in vtt_sections["위험 발언"]:
        ...
        if (
            risk
            and not risk.endswith("없습니다.")
            and not risk.startswith("특별한 주의사항이 없")
            and not "발견되지 않" in risk
            and not "확인되지 않" in risk
            and not "포함되어 있지 않" in risk
            and not "위험한 내용이 없" in risk
            and not "특별한 위험" in risk
            and not "부적절한 내용이 없" in risk
        ):
    ...
    html_content.extend(
        [
            ...
            + (" has-risks" if has_real_risks else " no-risks")
            + '">',
            '    <h2 class="category-title">위험 발언</h2>',
            '    <div class="risk-summary">',
            '        <div class="risk-icon">'
            + ("⚠️" if has_real_risks else "✅")
            + "</div>",
            "        <p>"
            + (
                "다음과 같은 위험 발언이 감지되었습니다."
                if has_real_risks
                else "위험 발언이 감지되지 않았습니다."
            )
            + "</p>",
            "    </div>",
        ]
    ...
    if has_real_risks:
        html_content.extend(['    <ul class="risk-list">'])
...
def format_chat_analysis(content):
    ...
    for section in sections:
        ...
        for line in lines:
            ...
            # 다른 카테고리 처리
            if current_category in categories and isinstance(
                categories[current_category], list
            ):
    ...
    for item in categories["위험 발언 및 주의사항"]:
        ...
        if (
            item
            and not item.endswith("없습니다.")
            and not item.startswith("특별한 주의사항이 없")
            and not "발견되지 않" in item
            and not "확인되지 않" in item
            and not "포함되어 있지 않" in item
            and not "위험한 내용이 없" in item
            and not "특별한 위험" in item
            and not "부적절한 내용이 없" in item
        ):
...
def format_lecture_analysis(content):
    ...
    if isinstance(content, dict):
        ...
        for i, result in enumerate(lecture_results, 1):
            ...
            if chunk_time_range:
                html_content.append(
                    f'        <span class="time-range">({chunk_time_range})</span>'
                )
            ...
            if segments:
                ...
                for segment in segments[:5]:  # 최대 5개만 표시
                    ...
                    text = (
                        segment.get("text", "")[:100] + "..."
                        if len(segment.get("text", "")) > 100
                        else segment.get("text", "")
                    )
                    html_content.append(
                        f"            <li><strong>{time_range}:</strong> {text}</li>"
                    )
                html_content.extend(["        </ul>", "        </div>", "    </div>"])
    ...
    # 각 섹션의 내용을 저장할 딕셔너리
    lecture_sections = {"주요 내용": [], "키워드": [], "분석": [], "학습 포인트": []}
...
def format_risk_analysis(content):
    ...
    if isinstance(content, dict):
        ...
        if not risk_results:
            ...
            for i, result in enumerate(risk_results, 1):
                ...
                improvement_suggestions = risk_analysis.get(
                ...
                if affected_segments:
                    ...
                    for segment in affected_segments:
                        ...
                        html_content.append(
                            f"            <li><strong>{time_range}:</strong> {text}"
                        )
                        if matched_risks:
                            html_content.append("                <ul>")
                            for risk in matched_risks:
                                ...
                                html_content.append(
                                    f"                    <li>위험: {risk_text} (신뢰도: {confidence:.2f})</li>"
                                )
                            html_content.append("                </ul>")
    ...
    # 각 섹션의 내용을 저장할 딕셔너리
    risk_sections = {"위험 발언 분석": [], "주의사항": [], "개선 제안": []}
...
def format_curriculum_matching_result(curriculum_matching):
    ...
    if segment_scores:
        ...
        for seg_score in segment_scores:  # 상위 20개만 표시
            ...
            preview = (
                seg_score.get("text", "")[:100] + "..."
                if len(seg_score.get("text", "")) > 100
                else seg_score.get("text", "")
            )
